...
*Main Modeling*
For our analyses, we used Random Forest modeling (Breiman, 2001). Random forest is an extension of decision tree modeling, whereby  

Random forest is well suited to the research questions that we had here because _________. In this study, we used the package randomForest in R (Liaw, 2018). 500 trees were grown as part of our random forest. 
We used the default value for "mtry," such that at each node, _____ predictors were candidates for splitting.

We partitioned the data before conducting the main analysis so that neither the training and testing dataset would not be disproportionately representative of high-achieving or low-achieving students. The training dataset consisted of 80% of the original data (n = ___ cases), whereas the testing dataset consisted of 20% of the original data (n = ____ cases).
