...
In this study, we used the package randomForest in R (Liaw, 2018). 500 trees were grown as part of our random forest. 
We used the default value for "mtry," such that at each node, _____ predictors were candidates for splitting.

We partitioned the data before conducting the main analysis so that neither the training and testing dataset would not be disproportionately representative of high-achieving or low-achieving students. The training dataset consisted of 80% of the original data (n = 400 cases), whereas the testing dataset consisted of 20% of the original data (n = 99 cases).

*Model Interpretation*
To interpret our findings, we examined three main things: (1) predictive accuracy of the random forest model, (2) variable importance, and (3) variance explained

We determined whether we had a strong model by examining the 
...
*Limitations*
    ...
    -   Listwise deletion of cases = potentially problematic because students with some missing data could differ in important ways from students with no missing data.
