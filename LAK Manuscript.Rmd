...
dd1 <- data.frame(data_test, rf_pred)
caret::RMSE(dd1$final_grade, dd1$rf_pred)

lm_m <- lm(formula = final_grade ~ pre_int + pre_uv + pre_percomp + time_spent + cogproc + social + posemo + negemo + n,
           data = data_train)
lm_pred <- predict(lm_m, data_test)
dd2 <- data.frame(data_test, lm_pred)
caret::RMSE(dd2$final_grade, dd2$lm_pred)
```

# 3. RESULTS 

The predictive accuracy of our random forest model was assessed by examining the difference between the predicted values for the testing dataset and the actual values. We found that the absolute value of the average difference between the predicted and actual value for final grades was 11.8%. This indicates... [add]
